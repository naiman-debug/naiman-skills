...
**[v0.1.0 pre-release is available](https://github.com/naiman-debug/naiman-skills/releases/tag/v0.1.0).** Download **Source code (zip)** from Assets and follow the installation guide. This is an early version for practice use; see [release notes](RELEASE_NOTES.md) and the validation limits below. The initial target is Codex with local file access. Other clients are not verified. Instructions and templates are primarily Chinese; you can ask Codex to respond in your preferred language.

Static checks and controlled practice-project runs cover the selected skills, including a scoped multi-turn Bubugao test. A clean host discovered all five selected skill folders. A new Codex desktop task explicitly read the candidate paths and ran all three main skills; an independent reviewer checked the tool outputs and resulting files. The complete update and rollback exercise also checked file replacement, retained local changes, and subsequent explicit calls. After publication, the actual v0.1.0 ZIP was downloaded, matched against the reviewed 58-file package, and installed in a separate practice directory; an independent desktop task explicitly read the installed paths and made read-only calls to all three main skills. Standalone CLI invocation, automatic discovery from the current nested test path, and other operating systems remain unverified; see the [project record](docs/项目记录.md) for scope and remaining limits.
